...
JOB_REGISTRY = [
    ...
    },
    {
        ...
        'description': 'Builds the weekly snapshot of every active brand: stage, readiness %, "now" status and a colored weekly history dot.',
        'schedule_human': 'Every Friday at 15:00 Asia/Kolkata',
    ...
    # ─── Data Sync (external APIs) ──────────────────────────────────────────
    {
        ...
        'description': 'Imports partners (doctors, clinics, etc.) from the external Ask Ayurveda CRM HTTP API and upserts the local Partner table.',
        'schedule_human': 'Every hour, 24/7',
        'cron': {'minute': 5},
        'timezone': IST_TZ,
    ...
    # ─── Maintenance ────────────────────────────────────────────────────────
    {
        ...
        'description': 'Continuously retries failed or stuck transcription / summarization / operator-feedback rows with exponential backoff (1m → 5m → 15m → 1h → 4h). Stops after 5 consecutive failures per row so we never burn OpenAI tokens on a permanently-broken item — those dead-ended rows surface here for a human to investigate.',
