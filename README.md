...
- Канбан-доска, задачи, комментарии.
...
- Все ключи (`OPENAI_API_KEY`, `PARTNERS_CRM_API_KEY`, `SECRET_KEY`) загружаются исключительно из переменных окружения.
- GitHub Push Protection включён в репозитории — секреты в коммитах будут отклонены автоматически.
- При смене сотрудников или подозрении на утечку — обязательно ротировать ключи: OpenAI, внешний CRM API.
