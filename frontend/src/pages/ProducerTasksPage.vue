<template>
  <q-page class="q-pa-md">
    ...
    <div class="row items-center q-mb-md q-gutter-sm">
      ...
      <q-btn unelevated color="green-8" icon="add" label="New Task"
    ...
    <q-card flat bordered style="border-radius:12px;overflow:hidden">
      ...
      >
          ...
          <q-td :props="props">
            <div class="row items-start q-gutter-xs">
              ...
              <div>
                ...
                <div v-if="props.row.description" class="text-caption text-grey-5 ellipsis" style="max-width:280px">
        ...
        <!-- Producer -->
        <template #body-cell-producer="props">
          <q-td :props="props">
            <template v-if="props.row.producer">
              <router-link :to="`/producers/${props.row.producer}`" class="producer-link" @click.stop>
                {{ props.row.producer_name }}
              </router-link>
              <div class="row items-center q-gutter-xs q-mt-xs" v-if="producerInfo(props.row.producer)">
                <div :style="priorityPill(producerInfo(props.row.producer)?.priority)"
                  style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:6px;">
                  {{ producerInfo(props.row.producer)?.priority_display }}
                </div>
                <div :style="`width:6px;height:6px;border-radius:50%;background:${coopDot(producerInfo(props.row.producer)?.cooperation_potential)};`">
                  <q-tooltip>Coop: {{ producerInfo(props.row.producer)?.cooperation_potential_display }}</q-tooltip>
                </div>
              </div>
            </template>
        ...
        <template #body-cell-due_date="props">
          <q-td :props="props">
            <template v-if="props.row.due_date">
              <div v-if="props.row.is_overdue && props.row.status !== 'done'" class="due-overdue">
                <q-icon name="warning" size="12px" /> {{ fmtDate(props.row.due_date) }}
              </div>
              <div v-else-if="isToday(props.row.due_date)" class="due-today">
...
import ProducerGlobalTaskDialog from 'src/components/ProducerGlobalTaskDialog.vue'
...
const columns = [
  ...
  { name: 'producer',    label: 'Producer',   field: 'producer', align: 'left',   sortable: false },
...
async function load() {
  ...
  try {
    ...
    const data = await store.fetchProducerTasks(params)
...
async function quickStatus(task, newStatus) {
  await store.updateProducerTask(task.id, { status: newStatus })
...
function confirmDelete(task) {
  $q.dialog({
    ...
    await store.deleteProducerTask(task.id)
    await store.refreshOpenTasksCount()
    load()
...
const producerMap = computed(() => {
  const m = {}
  for (const p of store.producers) m[p.id] = p
  return m
})
const producerInfo = (id) => producerMap.value[id] || null
...
}[p] || 'background:#FFF3E0;color:#E65100;')

const coopDot = (p) => ({
  strong: '#43A047', medium: '#1E88E5', weak: '#FF7043', no_response: '#9E9E9E',
}[p] || '#9E9E9E')
